...
export default function Getstarted(props: {
  data: ApiResponse<"/search/trending">;
}) {
  const { data } = props;
  ...
  return (
    <section className="flex flex-col justify-center items-center gap-5">
      <div className="flex flex-col justify-evenly items-center bg-blue rounded-xl p-10 gap-3">
        <h3 className="text-[25px] text-center font-semibold text-foreground max-w-[300px]">
          Get Started with KoinX for FREE
        </h3>
        <p className="text-[15px] text-center font-normal  text-foreground ">
        ...
        <button
          type="button"
          className=" px-5 py-2 font-medium text-[18px] rounded-lg  text-black bg-foreground outline-none border-none"
        >
            ...
            >
              <div className="flex justify-center items-center gap-4">
                ...
                <p className="text-[16px] font-medium text-black">
                  {d?.item?.name}({d?.item?.symbol})
                </p>
              </div>
              <span
                className={`text-[14px] w-[80px] font-mediu px-2 rounded-md ${
